...
| 模型 | c | MAPE | ±30% 带 | 判定 |
|---|---|---|---|---|
| 稀释公式 βc√K·∫√m dx（BvNW 标准式），cell=medNN | 1.29 先验 | **22.5%** | 7/10 | 达到文献天花板，边界线 10/11 差在带缘 |
...
2. **密集 regime（f>2.5）**：输出稀释估计 + `model_form="dilution_BvNW"` + `model_form_uncertainty=MAPE_25pct`，明示"真实运营残差可达 2×，用于数量级证伪而非定标"——**这是能力声明，不是失败**；
