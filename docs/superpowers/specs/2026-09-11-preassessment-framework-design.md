...
全国 572 名销售批量体检中发现：天津某销售的客户坐标录为（店名脱敏）： `(110.0, 110.0)`——纬度 110° 越过北极点，是采集端失败时的手敲占位值。它让该员工月里程虚报为 **15,946 km**（真实约 87 km），并一度污染了全城市聚合统计。规则拦截（bbox 校验）+ 几何拦截（kNN 离群飞点标记）可以在毫秒级自动抓出这类点。
...
| v1.4 | 2026-09-11 | **广州留出证伪框架自身 → V2.2**：十线（f=4.2–4.6）0/10 命中，"K不变"公理仅稀疏分区 regime 成立（房山 V≈K² 巧合骗过标定）；新增 regime 守护（f>2.5 拒答 REFUSED_*）、飞点 10km 绝对地板（密集城市 8× 相对判据误杀合法邻居）；契约 C7/C10 同步留痕 |