...
## 附录 C-PERF：性能契约（V2.1 修订，回应评审 §2.3 + 附加项④）

- **0.1.0 采用精确暴力 O(N²) NN**（评审裁决：正确性优先；单销售典型 100–500 店，亚秒级足够）。绝对规模性能承诺不在本版做出；可证明停止（ring-expansion + 网格边界下界）的网格加速随 0.2 落地，并必须携带与本暴力实现的等价性测试。
- 性能测试命名 **synthetic throughput benchmark**：合成吞吐 ≠ 真实数据最坏分布（后者在私有留出环境补测）。
- 门槛为**基线相对制**（记录 `tests/perf_baseline.json`，退化 > 2x 判失败），不用固定秒数做跨环境绝对门槛（CI 机器抖动会假红）；500 店 <5s 仅作防算法级错写的宽松冒烟。
- 复杂度声明与实现必须一致：README/BENCH 中 "bruteforce_v1" 显式标注，禁止口头宣称网格复杂度。
