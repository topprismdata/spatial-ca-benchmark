...
**广州：留出集证伪了框架自己，两次（故事三）。** 发布前用真实留出数据（海珠荔湾 10 条线，周访合同 f=4.2–4.6，实测 OSM 里程做参照）检验参考带：**0/10 命中，实测全部比带高 2–3 倍**。根因是"月总量与 K 无关"公理隐含日集合空间互斥假设，只在稀疏回访下成立——房山（V≈K²）恰好满足，通过了全部标定；广州密集重扫直接拆穿。v0.1 的修复是拒答；v0.2 的修复是**换语义**：密集 regime 改出**均匀场稀释上锚**（Steele/BvNW 极限的最大平滑尺度取值），单边政策带——紧凑片区必然低于锚，只有超锚 40% 的计划才是结构可疑信号。广州实测：人类基线 9/10 落锚带，唯一越缘的 08 线（超锚 60%）恰是已知结构最差的人类计划，其 SP 优化版落带；SP 优化计划 10/10 落带。中途还有一次自我证伪：曾以为 medNN 网格的 Stieltjes 积分达标（7/10），实现回归发现是量纲 bug 制造的假达标，整条冻结规则撤回。这个故事的要点：**留出验证第一次开火的对象应该是框架自己；数值达标在量纲检查面前不作数。**
...
| `NOT_ASSESSED_DEGENERATE_GEOMETRY` | 退化几何带 | 共线/零面积/单点：几何上无带可言 |
| （密集 regime 不再拒答） | 上锚带 | v0.2 起密集回访出**均匀场稀释上锚** + 单边带（见 §2 故事三） |
